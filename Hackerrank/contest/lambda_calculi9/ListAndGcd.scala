...
/**
 * <p>
 * <b>Problem Statement</b> We call an integer p>1 a prime number (or simply a
 * prime) if its only positive divisors are 1 and p. <br>
 ...
 */
object ListAndGdc {
  def intersection(list: List[List[Int]]): List[Int] =
  {
    list.foldLeft(toSortedPrimeFactorizedList(list.head))(_ intersect toSortedPrimeFactorizedList(_))
  }
   
  def toSortedPrimeFactorizedList(array: List[Int]): List[Int] =
  {
    val res = array match 
    {
      case x::y::l => List.fill(y)(x) ::: toSortedPrimeFactorizedList(l)
...
//    val lines = io.Source.stdin.getLines.toList
    ...
    val array = lines.drop(1).map(_.trim.split(' ').toList.map(_.toInt))
    val map = intersection(array).groupBy(_.toInt).mapValues(_.size)
    map.keySet.toList.sortWith(_ < _).foreach {k:Int => print(k + " " + map.get(k).get + " ") }
  }
}